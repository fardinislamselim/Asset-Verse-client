...
const AssetDetails = () => {
  ...
  if (isError || !asset) {
    return (
      <div className="flex flex-col items-center justify-center h-screen gap-4">
        ...
        <button className="btn btn-primary rounded-field" onClick={() => navigate(-1)}>
          Go Back
        </button>
      </div>
    );
  ...
  return (
    <div className="bg-base-100 min-h-screen pb-12">
      <div className="max-w-7xl mx-auto px-6">
               ...
               <div className="form-control w-full space-y-4">
                    <div className="space-y-1">
                      <label className="label py-0">
                         <span className="label-text font-semibold text-neutral">Purpose of Request</span>
                    ...
                    {!isOutOfStock && <p className="text-xs text-center text-gray-400 font-medium">Approval usually takes 24-48 hours.</p>}
                </div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-20">
          <div className="flex gap-8 border-b border-base-200 mb-8 overflow-x-auto">
            {["overview", "specifications", "reviews"].map((tab) => (
              ...
              >
                {tab}
                {activeTab === tab && (
                  <motion.div layoutId="underline-emp" className="absolute bottom-0 left-0 w-full h-0.5 bg-primary" />
                )}
              </button>
            ))}
          </div>
          ...
          <div className="min-h-[200px]">
            ...
            {activeTab === "specifications" && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid md:grid-cols-2 gap-6 bg-base-100 p-8 rounded-2xl border border-base-200 shadow-sm">
                 {specs.map((item, idx) => (
            ...
            {activeTab === "reviews" && (
               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                  {reviews.map((review) => (
                    <div key={review.id} className="flex gap-4 p-6 rounded-2xl bg-base-200/40 border border-base-200">
        ...
        {/* Related Assets */}
        <div className="mb-12">
           <h3 className="text-2xl font-bold mb-8 text-neutral">Related Assets</h3>
           <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              {[1, 2, 3].map((item) => (
                 <div key={item} className="card bg-base-100 shadow-md border border-base-200 hover:shadow-xl transition-all group overflow-hidden">
                    <figure className="h-40 bg-gray-50 relative group-hover:scale-105 transition-transform duration-500">
                       <div className="absolute inset-0 flex items-center justify-center text-gray-300 text-4xl">
                         <FaBoxOpen />
                       </div>
                    </figure>
                    <div className="card-body p-4">
                       <h4 className="font-bold text-lg mb-1 text-neutral">Related Asset #{item}</h4>
                       <p className="text-xs text-gray-500 mb-4">Similar to {asset.productName}</p>
                       <button className="btn btn-outline btn-sm w-full rounded-field group-hover:btn-primary transition-colors">View Details</button>
                    </div>
                 </div>
              ))}
           </div>
